...
namespace IPod {
    ...
    internal class AlbumListRecord : PhotoDbRecord {
        ...
        private List<PhotoAlbumRecord> albums = new List<PhotoAlbumRecord> ();

        public IList<PhotoAlbumRecord> Albums {
            get {
                return new ReadOnlyCollection<PhotoAlbumRecord> (albums);
            }
        }

        public void AddAlbum (PhotoAlbumRecord album) {
            albums.Add (album);
        }

        public void RemoveAlbum (PhotoAlbumRecord album) {
            albums.Remove (album);
        }
        ...
        public override void Read (BinaryReader reader) {
            ...
            for (int i = 0; i < numChildren; i++) {
                PhotoAlbumRecord record = new PhotoAlbumRecord (IsBE);
                record.Read (reader);
                albums.Add (record);
            }
        }
    ...
    internal class PhotoAlbumRecord : PhotoDbRecord {
        ...
        private Dictionary<int, PhotoAlbumItemRecord> items = new Dictionary<int, PhotoAlbumItemRecord> ();
        ...
        public IList<PhotoAlbumItemRecord> Items {
            get {
                return new ReadOnlyCollection<PhotoAlbumItemRecord> (new List<PhotoAlbumItemRecord> (items.Values));
            }
        }
        ...
        public PhotoAlbumRecord (bool isbe) : base (isbe) {
            this.Name = "mhba";
        ...
        public void AddItem (PhotoAlbumItemRecord item) {
            items[item.Id] = item;
        }
        ...
        public void RemoveItem (PhotoAlbumItemRecord item) {
        ...
        public override void Read (BinaryReader reader) {
            ...
            for (int i = 0; i < numAlbums; i++) {
                PhotoAlbumItemRecord item = new PhotoAlbumItemRecord (IsBE);
                item.Read (reader);
    ...
    internal class PhotoAlbumItemRecord : PhotoDbRecord {

        private int unknownOne;

        public int Id;

        public PhotoAlbumItemRecord (bool isbe) : this (isbe, 0) {
        }

        public PhotoAlbumItemRecord (bool isbe, int id) : base (isbe) {
            this.Name = "mhia";
            this.Id = id;
    ...
    public class PhotoDatabase {
        ...
        private PhotoAlbum masterAlbum;
        ...
        public IList<PhotoAlbum> Albums {
            get { return new ReadOnlyCollection<PhotoAlbum> (albums); }
        }
        ...
        public void Reload (bool createFresh) {
            ...
            using (BinaryReader reader = new BinaryReader (File.OpenRead (PhotoDbPath))) {
                ...
                foreach (PhotoAlbumRecord albumRecord in (albumSet.Data as AlbumListRecord).Albums) {
                    PhotoAlbum album = new PhotoAlbum (albumRecord, this);
        ...
        private PhotoAlbum CreateAlbum () {
            PhotoAlbumRecord record = new PhotoAlbumRecord (device.IsBE);

            AlbumListRecord albumList = dfr[PhotoDataSetIndex.AlbumList].Data as AlbumListRecord;
            albumList.AddAlbum (record);

            return new PhotoAlbum (record, this);
        }

        public PhotoAlbum CreateAlbum (string name) {
            PhotoAlbum album = CreateAlbum ();
            album.Name = name;
            albums.Add (album);
            return album;
        }
        ...
        private void SaveThumbnails (List<ImageNameRecord> existingNames, List<ImageNameRecord> newNames,
            ...
            //thumbPath.Substring
            //TODO: Syscall.truncate (thumbPath, (long) fileLength);
        }
